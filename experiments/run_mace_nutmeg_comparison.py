...
def run_spam_div_experiment(spam_rate, div_rate):
    ...
    temp_df = pd.DataFrame({
        ...
        'majority_accuracy': [accuracy_score(mace.labels_, truth_df['majority_truth']), accuracy_score(nutmeg.labels_[:, 0], truth_df['majority_truth'])],
        'minority_accuracy': [accuracy_score(mace.labels_, truth_df['minority_truth']), accuracy_score(nutmeg.labels_[:, 1], truth_df['minority_truth'])],
        'majority_f1': [f1_score(mace.labels_, truth_df['majority_truth']), f1_score(nutmeg.labels_[:, 0], truth_df['majority_truth'])],
        'minority_f1': [f1_score(mace.labels_, truth_df['minority_truth']), f1_score(nutmeg.labels_[:, 1], truth_df['minority_truth'])]
