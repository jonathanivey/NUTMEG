...
def run_disagreement_estimate_experiment(spam_rate, div_rate):
    print('Running with spam:', spam_rate, ' and div:', div_rate)

    # initialize model
    nutmeg = NUTMEG(n_iter=100)
    ...
    truth_df = synth_df.drop_duplicates(subset="task")

    # store our experiment results in a dataframe row
    temp_df = pd.DataFrame({
        'spamming_rate': [spam_rate],
        'divisiveness_rate': [div_rate],
        'true_divisiveness': [np.mean(truth_df['majority_truth'] != truth_df['minority_truth'])],
        'estimated_divisiveness': [np.mean(nutmeg.labels_[:, 0] != nutmeg.labels_[:, 1])]
    })
...
def experiment_wrapper(args):
    ...
    return run_disagreement_estimate_experiment(spam_rate, div_rate)
...
def main():
    ...
    experiment_df.to_csv('./results/figure_data/disagreement_estimate.csv', index=False)
